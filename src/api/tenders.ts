...
export async function listTenders(p: ListParams) {
  try {
    ...
    if (p.sort === "closing") {
      order = Prisma.sql`ORDER BY "closingDate" ASC NULLS LAST`;
    } else if (p.sort === "closing_desc") {
      order = Prisma.sql`ORDER BY "closingDate" DESC NULLS LAST`;
    } else if (p.sort === "published_asc") {
      order = Prisma.sql`ORDER BY "publishedDate" ASC NULLS LAST`;
    }
...
export async function getProvinces() {
  try {
    const rows = await prisma.$queryRaw<Array<{ province: string; count: number }>>`
...
export async function getStats() {
  try {
    ...
    const [catCount] = await prisma.$queryRaw<Array<{ count: number }>>`
      SELECT COUNT(DISTINCT "category")::int AS count FROM "Tender" WHERE "isOpportunity" = true AND "category" IS NOT NULL
    `;

    const [provCount] = await prisma.$queryRaw<Array<{ count: number }>>`
      SELECT COUNT(DISTINCT "province")::int AS count FROM "Tender" WHERE "isOpportunity" = true AND "province" IS NOT NULL
    `;
    ...
    return {
      ...
      latestPublishedDate: counts?.latestPublished
        ? (typeof counts.latestPublished === "string"
            ? counts.latestPublished
            : (counts.latestPublished as Date).toISOString())
        : null,
    };
  } catch (err) {
