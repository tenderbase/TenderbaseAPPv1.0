...
export interface MunicipalityIngestError { url: string | null; message: string; }
export interface MunicipalityIngestResult {
  source: string; pages: number; durationMs: number; fetched: number;
...
async function ensureMunicipality(adapter: MunicipalityAdapter): Promise<void> {
  ...
  await pool.query(
    ...
    [
      ...
      adapter.name,
      adapter.province,
      adapter.type ?? "LOCAL_MUNICIPALITY",
      adapter.sourceUrl,
    ],
  );
}
  ...
  try {
    ...
    for (const tender of res.tenders) {
      try {
        ...
        await prisma.scraperError.create({ data: { runId: run.id, url: tender.sourceUrl, message: detail.message.slice(0, 500) } });
      }
    }
    ...
    await prisma.scraperRun.update({
      where: { id: run.id },
      data: {
        status: counts.errors > 0 ? "ERROR" : "SUCCESS", finishedAt: new Date(), durationMs,
        pagesFetched: res.pages, fetchedCount: res.tenders.length,
        insertedCount: counts.inserted, updatedCount: counts.updated,
        unchangedCount: counts.unchanged, errorCount: counts.errors,
      },
    });
    return { source: adapter.id, pages: res.pages, durationMs, fetched: res.tenders.length, ...counts, errorDetails };
  } catch (error) {
    await prisma.scraperRun.update({ where: { id: run.id }, data: { status: "ERROR", finishedAt: new Date(), durationMs: Date.now() - startedAt } });
    throw error;
  }
}
